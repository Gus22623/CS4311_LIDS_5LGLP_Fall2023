...
class PacketCapture:
    ...
    def detect_alert(self, packet):
        if 'IP' in packet:
            ...
            if 'TCP' in packet:
                ...
                cursor.execute(f"SELECT * FROM config WHERE ip = '{dst}' AND FIND_IN_SET({dst_port}, ports)")
                result = cursor.fetchone()
                if result is None:
            ...
            if 'TCP' in packet:
                ...
                cursor.execute(f"SELECT * FROM config WHERE ip = '{src}' AND FIND_IN_SET({src_port}, ports)")
                result = cursor.fetchone()
                if result is not None:
                    self.trigger_alert(packet, "Failed Login Attempt", "High")

            # Check if the source or destination IP is Whitelisted
            cursor.execute(f"SELECT * FROM config WHERE (ip = '{src}' AND FIND_IN_SET('{dst}', whitelist)) OR (ip = '{dst}' AND FIND_IN_SET('{src}', whitelist))")
            result = cursor.fetchone()
            if result is not None:
                return
    ...
    def trigger_alert(self, packet, alert_type, severity):
        ...
        try:
            ...
            cursor.execute("INSERT INTO alerts (Lvl, Time, IP, Port, Description) VALUES (%s, %s, %s, %s, %s)",
                        (level, time, src, dst, alert_type))
            db.commit()

            print(f"ALERT [{severity}]: {alert_type} detected and saved to the database.")
            print(f"Time: {time}, Source: {src}, Destination: {dst}, Protocol: {protocol}")
